...
#include "TIG/Shaders/TIGShaderUtils.h"


void TestUAVCB::Initialize(wi::graphics::GraphicsDevice* Device)
{
	wi::renderer::LoadShader(wi::graphics::ShaderStage::CS, Shader, "TestUAVCBCS.cso");

	GPU_CREATE_CONST_BUFFER(Device, ConstBuffer, DataCB);
	...
	Device->SetName(&DataBuffer, "TestUAVB");
...
void TestUAVCB::Execute(wi::graphics::GraphicsDevice* Device, float DeltaTime)
{
	ExecuteCmd = Device->BeginCommandList(wi::graphics::QUEUE_COMPUTE);

	DataCB CB;
	CB.DeltaTime = DeltaTime;
	Device->UpdateBuffer(&ConstBuffer, &CB, ExecuteCmd, sizeof(DataCB));

	Device->BindUAV(&DataBuffer, 0, ExecuteCmd);
	Device->BindComputeShader(&Shader, ExecuteCmd);

	Device->BindConstantBuffer(&ConstBuffer, 0, ExecuteCmd);
...
std::unique_ptr<DataUAV[]> TestUAVCB::ReadBackResults(wi::graphics::GraphicsDevice* Device)
{
