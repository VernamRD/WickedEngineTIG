...
#include "TIG/Shaders/TestBulletCS.h"
...
public:
	...
	GameRender()
	{
		sprite = wi::Sprite(wi::helper::GetCurrentPath() + "/Content/logo_small.png");
...
const std::string BulletComponentManager_Default = "BulletComponentManager";
...
void CreateBullet(INT32 BulletCount)
{
	if (!wi::scene::GetScene().componentLibrary.Get<BulletComponent>(BulletComponentManager_Default))
	{
		wi::scene::GetScene().componentLibrary.Register<BulletComponent>(BulletComponentManager_Default);
	}

	for (INT32 i = 0; i < BulletCount; ++i)
	{
		auto NewBullet = wi::ecs::CreateEntity();
		auto ComponentManager = wi::scene::GetScene().componentLibrary.Get<BulletComponent>(
			BulletComponentManager_Default);
		auto Bullet = ComponentManager->Create(NewBullet);
		Bullet.BulletID = i;
	}
}

constexpr INT32 BulletsCount = 500000;
Game application;
                      ...
                      _In_ int nCmdShow)
{
	...
	HWND hWnd = CreateWindowW(wcex.lpszClassName, wcex.lpszClassName, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, 0,
...
void Game::Initialize()
{
	...
	wilog(std::to_string(wi::scene::GetScene().componentLibrary.Get<BulletComponent>(BulletComponentManager_Default)->
		GetComponentArray().size()).c_str());
	...
	wi::scene::LoadModel2("Content/BulletEmitter.wiscene", XMMatrixIdentity(), emitter_entity);

	wi::scene::LoadModel2("Content/Camera.wiscene", XMMatrixIdentity());
...
void Game::Update(float dt)
{
	...
	// 	auto scope = wi::profiler::BeginRangeCPU("BulletsUpdate");
	//
	// 	auto Bullets = wi::scene::GetScene().componentLibrary.Get<BulletComponent>(BulletComponentManager_Default)->
	// 	                                     GetEntityArray();
	// 	for (auto& BulletEntity : Bullets)
	// 	{
	// 		auto BulletEntityComponent = wi::scene::GetScene().componentLibrary.Get<BulletComponent>(
